    ...
    async send(p) {
      ...
      switch (c) {
        ...
        case 2: { const ack = P('deadbeef'); reply(cat([6, 0], ack, u32le(2500))); const prefix = hex(p.subarray(7, 13)); const ct = contacts.find(x => x.pub.startsWith(prefix)); const text = td.decode(p.subarray(13));
          ...
          if (ct && ct.type === 2 && p[1] === 1) setTimeout(() => { queue.push(cat([16, 28, 0, 0], P(ct.pub).subarray(0, 6), [1, 1], u32le(now()), te.encode(text === 'ver' ? 'MeshCore v1.12.0 (Build: 21 Sep 2026)' : text === 'neighbors' ? '7f3311aa2222: 6.75 45\n5d21aabb6666: -2.50 300\ncafebabe0001: 3.00 900' : text === 'region' ? '*\n eu F\n  bx\n   be^ F\n    be-vlg\n    be-vli\n    behss' : text.startsWith('get') ? '> ' + text.slice(4) + ': 869.618,250,11,5' : 'OK'))); this.push([0x83]); }, 900);
