...
C.addEventListener('newAdvert', (e) => { const c = e.detail; if (!c) return; S.pendingAdverts.set(c.pub, c); notice(t('ev.newNode', c.name, advType(c.type)), S.convs.get('status'), false); toast(t('ev.newNodeToast', c.name, advType(c.type)), '', 6000); if ($('#dlg-contacts').open) renderContactsDlg(); });
