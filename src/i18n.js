...
I18N.nl = {
  ...
  'ble.hintConnect': '{0} — controleer of de node niet nog met de telefoon-app verbonden is. Blijft het misgaan: verwijder de node in Windows (Instellingen › Bluetooth) en koppel opnieuw met de pincode (standaard 123456). Dat is nodig zodra Windows en de node niet meer dezelfde sleutels hebben, bijvoorbeeld na een herstart of herflash van de node, een pincodewijziging of een koppeling met een ander toestel.',
...
I18N.en = {
  ...
  'ble.hintConnect': '{0} — make sure the node is not still connected to the phone app. If it keeps failing: remove the node in Windows (Settings › Bluetooth) and pair again with the PIN (default 123456). That is needed whenever Windows and the node no longer share the same keys, for example after a reboot or reflash of the node, a PIN change or pairing with another device.',
...
I18N.fr = {
  ...
  'ble.hintConnect': '{0} — vérifiez que le nœud n\'est pas encore connecté à l\'application du téléphone. Si l\'échec persiste : supprimez le nœud dans Windows (Paramètres › Bluetooth) et jumelez-le à nouveau avec le code (123456 par défaut). C\'est nécessaire dès que Windows et le nœud n\'ont plus les mêmes clés, par exemple après un redémarrage ou un reflash du nœud, un changement de code ou un jumelage avec un autre appareil.',
