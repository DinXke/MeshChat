...
function mapInit(container) {
  ...
  mapObj = new maplibregl.Map({ container, style: mapStyle(), center, zoom: S.self && S.self.lat ? 9 : 7, attributionControl: { compact: true }, maxZoom: 16, minZoom: 3 });
  ...
  mapObj.on('click', 'nodes-circle', (e) => { const f = e.features && e.features[0]; if (!f) return; mapPopup(f, e.lngLat); });
  mapObj.on('mouseenter', 'nodes-circle', () => { mapObj.getCanvas().style.cursor = 'pointer'; });
  mapObj.on('mouseleave', 'nodes-circle', () => { mapObj.getCanvas().style.cursor = ''; });
...
function mapAddOverlays() {
  ...
  if (!mapObj.getLayer('nodes-circle')) mapObj.addLayer({ id: 'nodes-circle', type: 'circle', source: 'nodes', paint: { 'circle-radius': ['case', ['get', 'self'], 8, ['get', 'fav'], 7, 5.5], 'circle-color': ['get', 'color'], 'circle-stroke-width': ['case', ['get', 'self'], 3, 1.5], 'circle-stroke-color': ['case', ['get', 'stale'], 'rgba(255,255,255,.35)', '#ffffff'], 'circle-opacity': ['case', ['get', 'stale'], 0.55, 0.95] } });
  if (!mapObj.getLayer('nodes-label')) mapObj.addLayer({ id: 'nodes-label', type: 'symbol', source: 'nodes', layout: { 'text-field': ['get', 'name'], 'text-font': ['Noto Sans Medium'], 'text-size': 11.5, 'text-offset': [0, 1.1], 'text-anchor': 'top', 'text-optional': true, 'text-max-width': 12 }, paint: { 'text-color': mapIsDark() ? '#e6ebf0' : '#1c2530', 'text-halo-color': mapIsDark() ? 'rgba(14,17,22,.9)' : 'rgba(255,255,255,.9)', 'text-halo-width': 1.4 } });
...
function mapNodeFeatures() {
  ...
  for (const c of S.contacts.values()) { if (c.hidden || !c.lat || !c.lon) continue; feats.push({ type: 'Feature', geometry: { type: 'Point', coordinates: [c.lon, c.lat] }, properties: { pub: c.pub, name: cname(c), type: c.type, color: TYPE_COLORS[c.type] || TYPE_COLORS[0], self: false, fav: !!((c.flags & 1) || S.extras[c.pub]?.fav), stale: (c.lastAdvert || 0) < staleT, lastAdvert: c.lastAdvert || 0 } }); }
  if (S.self && S.self.lat && S.self.lon) feats.push({ type: 'Feature', geometry: { type: 'Point', coordinates: [S.self.lon, S.self.lat] }, properties: { pub: S.self.pub, name: S.self.name, type: 'self', color: TYPE_COLORS.self, self: true, fav: false, stale: false, lastAdvert: nowSecs() } });
...
function mapFocus(pub, zoom = 12) {
  ...
  mapShow(); const go = () => { mapObj.flyTo({ center: [c.lon, c.lat], zoom, duration: 900 }); mapObj.once('moveend', () => { const f = mapNodeFeatures().features.find(x => x.properties.pub === (c.pub || S.self.pub)); if (f) mapPopup(f, { lng: c.lon, lat: c.lat }); }); };
  if (mapReady) go(); else mapObj.once('load', go); return true;
}
function mapFitAll() { if (!mapObj) return; const fs = mapNodeFeatures().features; if (!fs.length) return; const b = new maplibregl.LngLatBounds(); for (const f of fs) b.extend(f.geometry.coordinates); mapObj.fitBounds(b, { padding: 60, maxZoom: 12, duration: 700 }); }
...
function mapShow() {
  ...
  mapAvailable().then(ok => { if (!ok) { mapSetNotice(mapAvailReason === 'offline' ? t('map_offline_nocache') : t('map_unavailable')); return; } mapSetNotice(''); mapInit($('#map')); setTimeout(() => mapObj && mapObj.resize(), 50); mapRefreshNodes(); });
}
